...
Shader "Lone Hero Studios/Rising Islands/LavaBall" {
    Properties {
        ...
        _SecondaryColor ("Secondary Color", Color) = (1,0.2068965,0,1)
        _PrimaryColor ("Primary Color", Color) = (0.7941176,0.3032306,0.1226211,1)
    ...
    SubShader {
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                float4 node_2981 = _Time + _TimeEditor;
                ...
                float2 node_6612 = ((float2(0.0,FlowMapValues)+node_2981.g*float2(0,0.1))*0.5);
                float4 _GlossTexture_var = tex2Dlod(_GlossTexture,float4(TRANSFORM_TEX(node_6612, _GlossTexture),0.0,0));
                float node_1561 = (_Gloss*_GlossTexture_var.r);
                float3 node_535 = float3(node_1561,node_1561,node_1561);
                v.vertex.xyz += ((objPos.rgb*node_535)*_Intensity);
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float2 node_6984 = (node_6016+node_2981.g*float2(0,0.1));
                float4 node_4936 = tex2D(_Normal,TRANSFORM_TEX(node_6984, _Normal));
                float2 node_5265 = (i.uv0+(node_6459*frac((node_458+0.5))));
                float2 node_6010 = (node_5265+node_2981.g*float2(0,0.1));
                float4 node_2487 = tex2D(_Normal,TRANSFORM_TEX(node_6010, _Normal));
                float node_1454 = 0.5;
...
////// Specular:
                ...
                float2 node_7179 = (node_6016+node_2981.g*float2(0,0.1));
                float4 node_2947 = tex2D(_Diffuse,TRANSFORM_TEX(node_7179, _Diffuse));
                float2 node_119 = (node_5265+node_2981.g*float2(0,0.1));
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.bitangentDir = normalize(cross(o.normalDir, o.tangentDir) * v.tangent.w);
                float4 objPos = mul ( _Object2World, float4(0,0,0,1) );
                float4 node_4689 = _Time + _TimeEditor;
                float4 _Flowmap_var = tex2Dlod(_Flowmap,float4(TRANSFORM_TEX(o.uv0, _Flowmap),0.0,0));
                float2 node_1479 = _Flowmap_var.rgb.rg;
                float FlowMapValues = node_1479.g;
                float2 node_6612 = ((float2(0.0,FlowMapValues)+node_4689.g*float2(0,0.1))*0.5);
                float4 _GlossTexture_var = tex2Dlod(_GlossTexture,float4(TRANSFORM_TEX(node_6612, _GlossTexture),0.0,0));
                float node_1561 = (_Gloss*_GlossTexture_var.r);
                float3 node_535 = float3(node_1561,node_1561,node_1561);
                v.vertex.xyz += ((objPos.rgb*node_535)*_Intensity);
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float node_5761 = frac(node_458);
                float2 node_6016 = (i.uv0+(node_6459*node_5761));
                float2 node_6984 = (node_6016+node_4689.g*float2(0,0.1));
                float4 node_4936 = tex2D(_Normal,TRANSFORM_TEX(node_6984, _Normal));
                float2 node_5265 = (i.uv0+(node_6459*frac((node_458+0.5))));
                float2 node_6010 = (node_5265+node_4689.g*float2(0,0.1));
                float4 node_2487 = tex2D(_Normal,TRANSFORM_TEX(node_6010, _Normal));
                float node_1454 = 0.5;
                float node_9116 = abs(((node_1454-node_5761)/node_1454));
                float3 normalLocal = lerp(node_4936.rgb,node_2487.rgb,node_9116);
...
////// Specular:
                ...
                float3 SecondaryColor = _SecondaryColor.rgb;
                float3 MainColor = _PrimaryColor.rgb;
                float2 node_7179 = (node_6016+node_4689.g*float2(0,0.1));
                float4 node_2947 = tex2D(_Diffuse,TRANSFORM_TEX(node_7179, _Diffuse));
                float2 node_119 = (node_5265+node_4689.g*float2(0,0.1));
                float4 node_9543 = tex2D(_Diffuse,TRANSFORM_TEX(node_119, _Diffuse));
                float3 DiffuseR = lerp(node_2947.rgb,node_9543.rgb,node_9116);
                float3 ColorOutput = lerp(SecondaryColor,MainColor,(DiffuseR*2.0*(1.0 - (1.0-max(0,dot(normalDirection, viewDirection))))));
                float3 diffuseColor = ColorOutput; // Need this for specular when using metallic
...
/// Final Color:
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.uv2 = v.texcoord2;
                float4 objPos = mul ( _Object2World, float4(0,0,0,1) );
                float4 node_8998 = _Time + _TimeEditor;
                float4 _Flowmap_var = tex2Dlod(_Flowmap,float4(TRANSFORM_TEX(o.uv0, _Flowmap),0.0,0));
                float2 node_1479 = _Flowmap_var.rgb.rg;
                float FlowMapValues = node_1479.g;
                float2 node_6612 = ((float2(0.0,FlowMapValues)+node_8998.g*float2(0,0.1))*0.5);
                float4 _GlossTexture_var = tex2Dlod(_GlossTexture,float4(TRANSFORM_TEX(node_6612, _GlossTexture),0.0,0));
                float node_1561 = (_Gloss*_GlossTexture_var.r);
                float3 node_535 = float3(node_1561,node_1561,node_1561);
                v.vertex.xyz += ((objPos.rgb*node_535)*_Intensity);
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                o.normalDir = UnityObjectToWorldNormal(v.normal);
                float4 objPos = mul ( _Object2World, float4(0,0,0,1) );
                float4 node_6151 = _Time + _TimeEditor;
                float4 _Flowmap_var = tex2Dlod(_Flowmap,float4(TRANSFORM_TEX(o.uv0, _Flowmap),0.0,0));
                float2 node_1479 = _Flowmap_var.rgb.rg;
                float FlowMapValues = node_1479.g;
                float2 node_6612 = ((float2(0.0,FlowMapValues)+node_6151.g*float2(0,0.1))*0.5);
                float4 _GlossTexture_var = tex2Dlod(_GlossTexture,float4(TRANSFORM_TEX(node_6612, _GlossTexture),0.0,0));
                float node_1561 = (_Gloss*_GlossTexture_var.r);
                float3 node_535 = float3(node_1561,node_1561,node_1561);
                v.vertex.xyz += ((objPos.rgb*node_535)*_Intensity);
            ...
            float4 frag(VertexOutput i) : SV_Target {
                ...
                float node_5761 = frac(node_458);
                float2 node_6016 = (i.uv0+(node_6459*node_5761));
                float2 node_7179 = (node_6016+node_6151.g*float2(0,0.1));
                float4 node_2947 = tex2D(_Diffuse,TRANSFORM_TEX(node_7179, _Diffuse));
                float2 node_5265 = (i.uv0+(node_6459*frac((node_458+0.5))));
                float2 node_119 = (node_5265+node_6151.g*float2(0,0.1));
                float4 node_9543 = tex2D(_Diffuse,TRANSFORM_TEX(node_119, _Diffuse));
                float node_1454 = 0.5;
                float node_9116 = abs(((node_1454-node_5761)/node_1454));
                float3 DiffuseR = lerp(node_2947.rgb,node_9543.rgb,node_9116);
